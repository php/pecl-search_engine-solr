...
static int solr_document_set_field(zval *objptr, solr_char_t *field_name, COMPAT_ARG_SIZE_T field_name_length, solr_char_t *field_value, COMPAT_ARG_SIZE_T field_value_length)
{
	...
	if (solr_fetch_document_entry(OBJ_FOR_PROP(objptr), &doc_entry) == SUCCESS)
	{
...
PHP_METHOD(SolrDocument, __construct)
{
	...
	zend_update_property_long(solr_ce_SolrDocument, OBJ_FOR_PROP(objptr), SOLR_INDEX_PROPERTY_NAME, sizeof(SOLR_INDEX_PROPERTY_NAME) - 1, document_index);
...
/* {{{ proto array SolrDocument::__serialize(void)
...
/* {{{ proto void SolrDocument::__unserialize(array data)
   New unserializer. */
#if PHP_VERSION_ID >= 80100
...
static void solr_add_child_input_documents_from_documents(HashTable * children, solr_document_t *new_doc_entry)
{
    SOLR_HASHTABLE_FOR_LOOP(children)
    {
        ...
        zend_call_method_with_0_params(OBJ_FOR_PROP(solr_doc), Z_OBJCE_P(solr_doc), NULL, "getinputdocument", &solr_input_doc);
...
PHP_METHOD(SolrDocument, getInputDocument)
{
	...
	if (solr_fetch_document_entry(OBJ_FOR_PROP(objptr), &old_doc_entry) == FAILURE) {
